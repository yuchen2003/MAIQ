...
class PPOLearner:
    ...
    def clone(self, batch: EpisodeBatch, t_env: int):
        ...
        if (t_env - self.last_target_update_episode) / self.args.target_update_interval_or_tau >= 1.0:
            self._update_targets()
            self.last_target_update_episode = t_env
        


    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, rewards=None):
        # Get the relevant quantities
        if rewards is None:
        ...
        #  old_pi[avail_actions[:, :-1] == 0] = -9999999
        old_pi[mask == 0] = 1.0
