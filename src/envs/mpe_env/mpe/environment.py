...
class MultiAgentEnv(gym.Env):
    ...
    def step(self, action_n):
        ...
        info_n = {'n': []}
        self.agents = self.world.policy_agents
        # print(f"Check env agent num!!!!!{len(self.agents)}")
        # assert False
        ...
        # print(f"check reward shape {np.array(reward_n), np.array(reward_n).shape}")
        # reward = np.sum(np.array(reward_n), axis=0)
        # print(f"and reward {reward}")

        # if self.shared_reward:
            # reward_n = [reward] * self.n
        with open("./test_reward.txt", 'a+') as f:
            f.write(f"{reward_n}\n")
