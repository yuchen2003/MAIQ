...
class RNNDiscriminator(nn.Module):
    def __init__(self, num_states, num_actions, n_agent, disc_type='decentralized',
        ...
        self.layers_after_rnn = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Linear(32, self.output_shape)
        )
    ...
    def forward(self, states, actions, hidden_state):
        """
        ...
        if self.use_noise:  # trick: add gaussian noise to discriminator
            x += th.normal(0, self.noise_std, size=x.shape)
        # for module in self._module_list:
        #     x = module(x)
        # return x
        fc1 =  th.nn.ReLU()(self.first_fc(x))
        hidden_in = hidden_state.reshape(-1, self.hidden_dim)
        hidden_state = self.rnn(fc1, hidden_in)

        x = self.layers_after_rnn(hidden_state)
