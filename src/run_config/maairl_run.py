...
from modules.maairl import REGISTRY as discriminator_REGISTRY
...
def run_maairl(_run, _config, _log):
    # check args sanity
        ...
        if buffer.can_sample(args.batch_size):
            ...
            for d_iter in range(args.d_iters):
                if disc_type == 'centralized':
                    ...
                    gen_r = train(args, discriminator, learner, episode_sample, expert_sample, logger, runner.t_env, disc_type,
                    ...
                    for k in range(env_info["n_agents"]):
                        if gen_r is None:
                            gen_r = train(args, discriminator[k], learner, episode_sample, expert_sample, logger, runner.t_env,
...
def train(args, discriminator, learner, episode_sample, expert_sample, logger, t_env, disc_type, n_agent: int = None,
    ...
    log_prob_episode = learner.get_log_prob(episode_sample)
    log_prob_expert = learner.get_log_prob(expert_sample)
